...
const width=290;   //altezza e larghezza della sala
const height= 400;
const seats = [
...
//questo componente accede al contesto, per poter usare lo stato, modificarlo con dispatch(), visualizzare il risultato
function InteractiveMap(){
    const[state, dispatch] = useContext(StateContext);
    //prende i contenuti dello state corrente, ma non lo modifica
    const venduti= state.venduti; //vettore venduti dello stato
    const cart= state.cart; //vettore cart dello stato
    const fares = state.fares;

    return (
        <svg width="100%" height="100%" viewBox= {`0 0 ${width} ${height}`} style={{marginLeft: '15px'}}>
            {seats.map(seat =>
                {
                    const occupied = venduti.filter(p => p.seatID === seat.seatID);
                    const selected = cart.filter(p => p.seatID === seat.seatID);
                    const price = fares.filter(t => t.tariffa==="intero")[0].price;
                    if(occupied.length!==0)
                        return <Seat key={seat.seatID} seatID={seat.seatID} x={seat.x} y={seat.y} size={seat.size} occupied={true} selected={false}
                                     onCLick={() => {}}/>;
                    if(selected.length!==0)
                        return <Seat key={seat.seatID} seatID={seat.seatID} x={seat.x} y={seat.y} size={seat.size} occupied={false} selected={true}
                                     onCLick={()=>dispatch(unselect(seat.seatID))}/>;
                    else
                        return <Seat key={seat.seatID} seatID={seat.seatID} x={seat.x} y={seat.y} size={seat.size} occupied={false} selected={false}
                                      onCLick={()=>dispatch(select(seat.seatID, price))}/>;
                }
            )}
        </svg>)
}
...
function Seat({seatID, x, y, size, occupied, selected, onCLick}){
    if(occupied) return (
        <g key={seatID} id={seatID} stroke='black' fill='darkred' onClick={onCLick}>
            <ellipse cx={x+size/2} cy={y} rx={size/2} ry={size/4}/>
            <rect key={seatID} x={x} y={y} width={size} height={size} />
            <text x={x+size/2} y={y+2*size/3} textAnchor="middle" fontSize="8px" stroke="none" fill="black">{seatID}</text>
        </g>
    )
    if(selected) return (
        <g key={seatID} id={seatID} stroke='black' fill='orange' onClick={onCLick}>
            <ellipse cx={x+size/2} cy={y} rx={size/2} ry={size/4}/>
            <rect key={seatID} x={x} y={y} width={size} height={size} />
            <text x={x+size/2} y={y+2*size/3} textAnchor="middle" fontSize="8px" stroke="none" fill="black">{seatID}</text>
        </g>
        ...
        <g key={seatID} id={seatID} stroke='black' fill='lightgrey' onClick={onCLick}>
            <ellipse cx={x+size/2} cy={y} rx={size/2} ry={size/4}/>
            <rect key={seatID} x={x} y={y} width={size} height={size} />
            <text x={x+size/2} y={y+2*size/3} textAnchor="middle" fontSize="8px" stroke="none" fill="black">{seatID}</text>
        </g>
